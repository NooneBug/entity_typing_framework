...
class ClauseEnhancer(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.gather_literal_indices = torch.tensor(gather_literal_indices, device=torch.device(self.device))
        # self.gather_literal_indices = torch.tensor(gather_literal_indices)

        self.signs = torch.tensor(signs, dtype=torch.float32, device=torch.device(self.device))
        # self.signs = torch.tensor(signs, dtype=torch.float32)

        self.register_parameter(
            name='clause_weight',
            param=torch.nn.Parameter(torch.tensor(self.initial_weight, device=torch.device(self.device))))
    ...
    def grounded_clause(self, ground_atoms: torch.Tensor) -> torch.Tensor:
        ...
        selected_predicates = ground_atoms[..., self.gather_literal_indices].to(torch.device(self.device)) # [b, l]
    ...
    def forward(self, ground_atoms: torch.Tensor) -> (torch.Tensor, torch.Tensor):
        ...
        # [b, 2|U|+|B|]
        scattered_delta = torch.zeros_like(ground_atoms, device=torch.device(self.device))
        # scattered_delta = torch.zeros_like(ground_atoms)
        scattered_delta[..., self.gather_literal_indices] = delta
        # print(self.string,' : delta ', str(delta))
