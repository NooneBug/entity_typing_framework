...
def generate_constraints_cross_dataset(all_types, new_types, filepath = None, weight='_', src2dst={}):
    # create ontology tree from all_types
    types_src = list(src2dst.keys())
    # create cross dataset clauses
    direct_clauses = [f'{weight}:{label2pred(t_src)},{label2pred(t_dst)}' for t_src, t_dst in src2dst.items() if t_dst]
    # TODO: add trasversal (with src2dst_clause parameter it would replace the line above)
    new_types_unmapped = list(set(new_types) - set(src2dst.values()))
