...
from entity_typing_framework.EntityTypingNetwork_classes.projectors_copy import ClassifierForCrossDatasetTraining
...
class KENNClassifierForCrossDatasetTraining(ClassifierForCrossDatasetTraining):
  ...
  def __init__(self, **kwargs):
    super().__init__(**kwargs)
    self.sig = Sigmoid()
    # self.src2dst_clauses = self.read_src2dst_clauses(kwargs['src2dst_clauses'])
  
  def read_src2dst_clauses(self, filepath):
  ...
  def get_kwargs_incremental_projector(self, **kwargs):
    ...
    if 'clause_file_path' not in kwargs:
      clause_file_path = f"kenn_tmp/{kwargs['kb_mode']}_cross_dataset_clause_file.txt"
    else:
      clause_file_path = kwargs['clause_file_path']
    cw = '_' if kwargs['learnable_clause_weight'] else kwargs['clause_weight']

    
    kenn_utils.generate_constraints_cross_dataset(all_types=all_types,
                                                  new_types=new_types,
                                                  filepath=clause_file_path,
                                                  weight=cw,
                                                  src2dst=self.src2dst_clauses)
  ...
  def forward(self, input_representation):
    ...
    return self.sig(prekenn_incremental), self.sig(postkenn_incremental)
